...
get_vending_apk () {
  "$ADB" -s "$1" pull $("$ADB" -s "$1" shell pm path com.android.vending| head -n 1 | cut -d ":" -f2) base.apk
}

update_frida_scripts() {
  APKTOOL=./tools/apktool
  "$APKTOOL" d -f -q -r -o base base.apk

  CLIENT_INTEGRITY_RECEIVER_CLASS=$(grep -nl "com.google.android.play.core.integrity.protocol.IRequestDialogCallback" $(grep -nrl "com.google.android.play.core.integrity.protocol.IIntegrityService" base/ | grep -ie "smali_classes./....\.smali")| cut -d "/" -f3 | cut -d "." -f1)
  CLIENT_CALLBACK_CLASS=$(grep -nl "transactOneway" $(grep -nrl "com.google.android.play.core.integrity.protocol.IIntegrityServiceCallback" base/)| cut -d "/" -f3 | cut -d "." -f1)
